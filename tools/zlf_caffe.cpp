...
int time() {
  ...
  caffe::Phase phase = get_phase_from_flags(caffe::TRAIN);//default phase is TRAIN
  vector<string> stages = get_stages_from_flags();
  ...
  double average_time_per_layer_sum=0;
  for (int i = 0; i < layers.size(); ++i) {
    average_time_per_layer_sum += forward_time_per_layer[i] / 1000 /
      FLAGS_iterations;
  }
